...
function crearDivResultado($num){
    if (!file_exists("tablas/tabla_$num.txt")){
        return "<div class='result'><p>La tabal del $num no ha sido creada</p></div>";
    }else{
        $file = fopen("tablas/tabla_$num.txt","r");
        $divResultado = "<div class='result'><ul>";
        while(!feof($file)){
            $divResultado .= "<li>".fgets($file)."</li>";
        }
        fclose($file);
        $divResultado .="</ul></div>";
        return $divResultado;
